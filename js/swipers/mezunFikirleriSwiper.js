...
mezunFikirData.forEach((data) => {
  mezunFikirleriWrapper.innerHTML += `
        <div class="swiper-slide flex flex-col md:flex-row md:p-9">
          ...
          <div class="w-2/4 flex flex-col justify-center mx-auto pl-6 pr-20">       
            <p class="text-xl md:text-6xl mb-6 text-[#1A265C]">
                ${data.description}
            </p>
            <p class="font-semibold text-lg md:text-6xl italic text-[#617EFF]">
                ${data.fullName}
            </p>
            <p class="font-medium text-lg md:text-6xl italic text-[#617EFF]">
                ${data.work}
            </p>            
          </div>
        </div>
